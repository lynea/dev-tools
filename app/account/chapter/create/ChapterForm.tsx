...
import { chapterSchema } from '@/lib/schema/entityGroup.schema'
...
export const ChapterForm = ({ entityGroups }: { entityGroups: Entity[] }) => {
    ...
    const onSubmit = async (data: z.infer<typeof chapterSchema>) => {
        try {
            await createChapter(data).then(() => {
                form.reset()
                toast('Chapter group created successfully')
            })
        } catch (error) {
            console.error('error creating chapter', error)
            toast(
                'Something went wrong while creating the chapter. Please try again.'
            )
        }
    }
